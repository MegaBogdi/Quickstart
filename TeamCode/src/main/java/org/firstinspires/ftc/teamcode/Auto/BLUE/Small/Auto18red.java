...
@Configurable
@Autonomous(name = "Autonomie 18 red")
public class Auto18red extends CommandOpMode {
    ...
    public void initialize() {
        ...
        leverPose = utils.mirror(leverPose);
        retryPose = utils.mirror(retryPose);
        preLeverPose = utils.mirror(preLeverPose);
        shootPose = utils.mirror(shootPose);
        spike1Pose = utils.mirror(spike1Pose);
        preSpike2Pose = utils.mirror(preSpike2Pose);
        spike2Pose = utils.mirror(spike2Pose);
        exitPose = utils.mirror(exitPose);
...
//========================== LEVER SEQUENCE ===========================
                new ParallelDeadlineGroup(
                        new SequentialCommandGroup(
                                new FollowPathCommand(follower, shootToLeverPath),new InstantCommand(()->IO.resetTime(timer.milliseconds())),
                        ...
                        new RepeatCommand(
                                new ConditionalCommand(
                                        new SequentialCommandGroup(new FollowPathCommand(follower, retryLever),new InstantCommand(()->IO.resetTime(timer.milliseconds()))),
                                        new InstantCommand(),
                                        ()->IO.isIntakeLate(timer.milliseconds()) && utils.intake_active
                                ))
                ),
